...
climate.dir = '~/Climate/CIAS/Australia/5km/baseline.76to05/bioclim/'; setwd(climate.dir) #define the climate directory
...
species = unique(occur$spp) #get list of the unique spp names
...
for (spp in species) { cat(spp,'\n') #if crashes change this number to the number of spp that have worked (by checking the outdata)
	tt = which(occur$spp == spp); pnts = cbind(occur$Lon[tt],occur$Lat[tt]) #select the points for the single species only
	...
	write.csv(x=outdata,file="summary.data/ENFA.summary.data.csv",row.names = FALSE)
}
