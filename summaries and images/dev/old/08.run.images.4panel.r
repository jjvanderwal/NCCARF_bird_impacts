...
png(paste(spp,'.png',sep=''),width=dim(base.asc)[1]*2+20, height=dim(base.asc)[2]*2+250, units='px', pointsize=20, bg='white')
	par(mar=c(3,3,3,2),mfrow=c(2,2),cex=1,oma=c(3,0,1,0))
	
	mat = matrix(c(6,5,5,5,5,5,
				1,1,1,2,2,2,
				1,1,1,2,2,2,
				1,1,1,2,2,2,
				3,3,3,4,4,4,
				3,3,3,4,4,4,
				3,3,3,4,4,4),nr=7,nc=6,byrow=TRUE) #create a layout matrix for images
	...
	mtext('Current Climate Space', line=1,  side=1, cex=1.5)
    ...
    plot(states, lwd=0.5, ann=FALSE,axes=FALSE, add=TRUE,border="grey60")
	if (length(tpolys)>0) {plot(tpolys, lwd=2, ann=FALSE,axes=FALSE, add=TRUE)}
	mtext ('Current Emissions Trajectory - 2085', line=1,side=1,cex=1.5)
		
	image(base.asc, ann=FALSE,axes=FALSE,col='white', zlim=zlim, xlim=xlim,ylim=ylim)
	clip(xlim[1],xlim[2],ylim[1],ylim[2])
	image(RCP3PD.asc, ann=FALSE,axes=FALSE,col=cols, zlim=zlim, xlim=xlim,ylim=ylim,xpd=FALSE,add=TRUE)
	plot(states, lwd=0.5, ann=FALSE,axes=FALSE, add=TRUE,border="grey60")
    if (length(tpolys)>0) {plot(tpolys, lwd=2, ann=FALSE,axes=FALSE, add=TRUE)}	
	mtext ('Very low Emissions - 2085', line=1,side=1,cex=1.5)
	
	image(base.asc, ann=FALSE,axes=FALSE,col='white', zlim=zlim, xlim=xlim,ylim=ylim)
	clip(xlim[1],xlim[2],ylim[1],ylim[2])
	image(RCP6.asc, ann=FALSE,axes=FALSE,col=cols, zlim=zlim, xlim=xlim,ylim=ylim,xpd=FALSE,add=TRUE)
	plot(states, lwd=0.5, ann=FALSE,axes=FALSE, add=TRUE,border="grey60")
    if (length(tpolys)>0) {plot(tpolys, lwd=2, ann=FALSE,axes=FALSE, add=TRUE)}
	mtext ('Reduced Emissions - 2085', line=1,side=1,cex=1.5)
